...
public class MainActivity extends AppCompatActivity {
    ...
    private void startWorkoutTimer() {
        ...
        exerciseTimer = new CountDownTimer(exerciseDurationSeconds * 1000, 1000) {
            ...
            public void onTick(long l) {
                ...
                timerDurationTextView.setText(formatTimeString(l));
                if (stop) {
                    stop = false;
                    stop();
                }
    ...
    public void onSetTimerButtonPressed(View view) {
        String exerciseDurationString = exerciseDuration.getText().toString().trim();
        String restDurationString = restDuration.getText().toString().trim();

        if (TextUtils.isEmpty(exerciseDurationString)) {
            showToast("Please enter Workout time");
            return;
        }

        if (TextUtils.isEmpty(restDurationString)) {
            showToast("Please enter Rest time");
            return;
        }

        restDurationSeconds = Integer.parseInt(restDurationString);
        exerciseDurationSeconds = Integer.parseInt(exerciseDurationString);
    ...
    public void sendNotification(String message) {
        ...
        PendingIntent pendingIntentStop = PendingIntent.getBroadcast(this, 0, intentAction, PendingIntent.FLAG_UPDATE_CURRENT);
